...
const DEFAULT_BINDS: Dictionary = {
	"move_left": [{"type": "key", "keycode": KEY_A}, {"type": "joy_button", "button_index": 14}],  # DPad Left
	"move_right": [{"type": "key", "keycode": KEY_D}, {"type": "joy_button", "button_index": 15}],  # DPad Right
	"move_forward": [{"type": "key", "keycode": KEY_W}, {"type": "joy_button", "button_index": 12}],  # DPad Up
	"move_backward":  # DPad Down
	[{"type": "key", "keycode": KEY_S}, {"type": "joy_button", "button_index": 13}],
	"ui_accept": [{"type": "key", "keycode": KEY_SPACE}, {"type": "joy_button", "button_index": 0}],  # A
	"show_radial_menu":  # LB
	[{"type": "key", "keycode": KEY_Q}, {"type": "joy_button", "button_index": 4}]
}
...
@onready
var km_actions_container: VBoxContainer = $VBox/TabContainer/KeyboardMouse/ScrollContainer/ActionsVBox
@onready
var gp_actions_container: VBoxContainer = $VBox/TabContainer/Gamepad/ScrollContainer/ActionsVBox
...
func _build_km_rows() -> void:
	...
	for action_name_sn: StringName in actions:
		...
		for slot in range(max_slots):
			...
			var slot_index := slot
			btn.pressed.connect(
				func() -> void: _on_slot_rebind_pressed(action_name, slot_index, btn, "km")
			)
			row.add_child(btn)
			buttons.append(btn)
			...
			clear_btn.pressed.connect(
				func() -> void: _on_clear_slot_pressed(action_name, slot_index, "km")
			)
			row.add_child(clear_btn)
			clear_buttons.append(clear_btn)
		_rows[action_name] = {
			"row": row, "label": label, "buttons": buttons, "clear_buttons": clear_buttons
		}
...
func _build_gamepad_rows() -> void:
	...
	for action_name_sn: StringName in actions:
		...
		for slot in range(max_slots):
			...
			var slot_index := slot
			btn.pressed.connect(
				func() -> void: _on_slot_rebind_pressed(action_name, slot_index, btn, "gp")
			)
			row.add_child(btn)
			buttons.append(btn)
			var clear_btn: Button = Button.new()
			...
			clear_btn.custom_minimum_size = Vector2(30, 0)
			clear_btn.pressed.connect(
				func() -> void: _on_clear_slot_pressed(action_name, slot_index, "gp")
			)
			row.add_child(clear_btn)
			clear_buttons.append(clear_btn)
		_rows_gamepad[action_name] = {
			"row": row, "label": label, "buttons": buttons, "clear_buttons": clear_buttons
		}
...
func _refresh_action_row(action_name: String) -> void:
	...
	for i in range(max_slots):
		...
		if i < events.size():
			...
			btn.text = "—"
		btn.button_pressed = (
			_capture_action == action_name and _capture_slot == i and _capture_input_type == "km"
		)
...
func _refresh_action_row_gamepad(action_name: String) -> void:
	...
	for i in range(max_slots):
		...
		if i < events.size():
			...
			btn.text = "—"
		btn.button_pressed = (
			_capture_action == action_name and _capture_slot == i and _capture_input_type == "gp"
		)
...
func _on_slot_rebind_pressed(
	action_name: String, slot: int, button: Button, input_type: String
) -> void:
	if (
		_capture_action == action_name
		and _capture_slot == slot
		and _capture_input_type == input_type
	):
		_cancel_capture()
		return
...
func _event_to_dict(ev: InputEvent) -> Dictionary:
	if ev is InputEventKey:
		...
		return {"type": "mouse", "button_index": mb.button_index}
	elif ev is InputEventJoypadButton:
		var jb := ev as InputEventJoypadButton
		return {"type": "joy_button", "button_index": jb.button_index}
	return {}
