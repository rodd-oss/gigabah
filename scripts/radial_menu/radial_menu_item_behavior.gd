...
func input(event: InputEvent)->void:
	...
	if get_radial_menu().enable_click && _howered && event.is_action_pressed("radial_menu_click"):
		_pressed = true
		on_press.emit()
		get_radial_menu().queue_redraw()
	if get_radial_menu().enable_click && _pressed && event.is_action_released("radial_menu_click"):
		_pressed = false
		on_release.emit()
		get_radial_menu().queue_redraw()
