...
static func create_segment_polygon(
	center: Vector2,
	radius_a: float,
	radius_b: float,
...
static func segment_releative_point(
	center: Vector2,
	radius_a: float,
	radius_b: float,
	radius_padding: float,
	...
	return (
		...
		+ (
			Vector2(
				(
					(radius_a + radius_padding)
					+ (
						((radius_b - radius_padding) - (radius_a + radius_padding))
						/ 2
						* (radius_t + 1)
					)
				),
			...
			. rotated(
				(
					(start_angle + angle_padding)
					+ (
						((end_angle - angle_padding) - (start_angle + angle_padding))
						/ 2
						* (angle_t + 1)
					)
				)
